...
//MAXWIDTH、MAXHEIGHT、INITLEN 以字符�??
#define MAXWIDTH (30)
#define MAXHEIGHT MAXWIDTH
#define INITLEN (3)  //�?吃蛇的初始长�? 

//程序�?用到的各种字符，以及它们的�?�色和类型（以数字表示）
struct{
    char *ch;
    int color;
    char type;
...
int headerIndex, tailIndex;  //蛇头蛇尾对应的snakeMap�?的索引（下标�?
HANDLE hStdin;  //控制台句�?
...
void move(char direction){
    ...
    int newHeaderPreIndex;  //新蛇头坐标以前�?�应的索�?
    int newHeaderPreX, newHeaderPreY;  //新蛇头的索引以前对应的坐�?
    int newHeaderPreType;  //新蛇头以前的类型
    int oldTailX, oldTailY;  //老蛇尾坐�?
    // -----------------------------------------------
    //新蛇头的坐标
    switch(direction){
        case 'w':
...
void init(){
    ...
    int xCenter = MAXHEIGHT%2==0 ? MAXHEIGHT/2 : MAXHEIGHT/2+1;
    int yCenter = MAXWIDTH%2==0 ? MAXWIDTH/2 : MAXWIDTH/2+1;
    CONSOLE_CURSOR_INFO cci;  //控制台光标信�?
